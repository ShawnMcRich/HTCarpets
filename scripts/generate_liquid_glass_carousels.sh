...
RENDERER="${PROJECT_DIR}/docs/liquid-glass-carousel-renderer.html"
OUTPUT_DIR="${PROJECT_DIR}/public/media/carousel-info"
DRIVER_PORT="4455"
...
for sku in "${SKUS[@]}"; do
  ...
  READY_PAYLOAD='{"script":"const carpet=document.getElementById(\"carpet\"); const logo=document.getElementById(\"brand-logo\"); const cards=[...document.querySelectorAll(\".glass-card\")]; return {carpet:Boolean(carpet && carpet.complete && carpet.naturalWidth),logo:Boolean(logo && logo.complete && logo.naturalWidth),overflow:cards.some(card=>card.scrollHeight>card.clientHeight||card.scrollWidth>card.clientWidth),errors:window.__renderErrors || [],width:innerWidth,height:innerHeight};","args":[]}'
  ...
  if [[ "$(jq -r '.value.carpet and .value.logo' <<<"${READY_RESPONSE}")" != "true" ]]; then
  ...
  if [[ "$(jq -r '.value.errors | length' <<<"${READY_RESPONSE}")" != "0" ]]; then
    echo "Browser error detected for ${sku}" >&2
    jq '.value.errors' <<<"${READY_RESPONSE}" >&2
    exit 1
  fi

  SCREENSHOT_RESPONSE="$(curl -sS "${DRIVER_URL}/session/${SESSION_ID}/screenshot")"
  jq -r '.value' <<<"${SCREENSHOT_RESPONSE}" | base64 --decode >"${OUTPUT_DIR}/${slug}-liquid-glass-info.png"
  echo "Generated ${slug}-liquid-glass-info.png"
done

echo "Generated ${#SKUS[@]} liquid-glass carousel slides in ${OUTPUT_DIR}"
