...
RENDERER="${PROJECT_DIR}/docs/reel-cta-renderer.html"
OUTPUT_DIR="${PROJECT_DIR}/public/media/reel-ctas"
DRIVER_PORT="4452"
...
SKUS=(
  ...
  HT-KSH-0003 HT-QOM-0004 HT-SHB-0001 HT-TBZ-0001
)
...
for sku in "${SKUS[@]}"; do
  ...
  READY_PAYLOAD='{"script":"const image=document.getElementById(\"logo\"); return {ready:document.readyState, loaded:Boolean(image && image.complete && image.naturalWidth), width:window.innerWidth, height:window.innerHeight};","args":[]}'
  ...
  if [[ "$(jq -r '.value.loaded' <<<"${READY_RESPONSE}")" != "true" ]]; then
    echo "Logo did not load for ${sku}" >&2
    exit 1
  fi

  SCREENSHOT_RESPONSE="$(curl -sS "${DRIVER_URL}/session/${SESSION_ID}/screenshot")"
  jq -r '.value' <<<"${SCREENSHOT_RESPONSE}" | base64 --decode >"${OUTPUT_DIR}/${slug}-reel-cta.png"
